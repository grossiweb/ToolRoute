// Shared MCP-skill contribution write logic.
//
// Extracted from /api/contributions so /api/report can call it in-process
// instead of making an HTTP round-trip to itself (which could throw uncaught
// on a non-JSON sub-response and surface as a generic 500). processContribution
// returns { status, body } so each route wraps it in its own NextResponse.
//
// Model telemetry does NOT flow through here — that is /api/report/model.
...
export function validatePayload(payload: any, type: string): ValidationResult {
...
export async function processContribution(
  ...
  const { agent_identity_id, contribution_type, payload, proof_type = 'self_reported' } = input

  const validTypes = ['run_telemetry', 'comparative_eval', 'fallback_chain', 'benchmark_package']
  if (!validTypes.includes(contribution_type)) {
    return { status: 400, body: errorBody(`Invalid contribution_type. Must be one of: ${validTypes.join(', ')}`, undefined, undefined, 'GET /api/contributions for the full request schema') }
  }
  ...
  if (!validation.valid) {
    ...
    if (looksLikeModelReport) {
      return { status: 400, body: errorBody(validation.error || 'Invalid payload', 'This payload looks like LLM model telemetry, not MCP skill telemetry. /api/contributions only accepts skill executions (requires skill_id or skill_slug). For model executions, use POST /api/report/model.', 'POST /api/report/model', 'GET /api/report/model for the model telemetry schema') }
    }
    return { status: 400, body: errorBody(validation.error || 'Invalid payload') }
  }

  // Resolve or create contributor — null-guarded (no contrib!.id crash).
  ...
  if (agent_identity_id) {
    ...
    if (agent?.contributor_id) {
      ...
      const { data: contrib, error: contribErr } = await supabase
        ...
        .single()
      if (!contrib) {
        console.error('[contributions] contributor insert failed (agent path):', contribErr)
        return { status: 500, body: errorBody('Could not record contribution', 'Failed to initialize the contributor record. This has been logged; please retry.') }
      }
    ...
    const { data: contrib, error: contribErr } = await supabase
      ...
      .single()
    if (!contrib) {
      console.error('[contributions] contributor insert failed (anon path):', contribErr)
      return { status: 500, body: errorBody('Could not record contribution', 'Failed to initialize the contributor record. This has been logged; please retry.') }
    }
    contributorId = contrib.id
  }
  ...
  if (!event) {
    console.error('[contributions] event insert failed:', eventErr)
    return { status: 500, body: errorBody('Failed to record contribution', 'Internal database error. Retry the request; if it persists, check /api/health and the Vercel logs.') }
  }
...
// ── Scoring heuristics (moved verbatim from the route) ───────────────────────
