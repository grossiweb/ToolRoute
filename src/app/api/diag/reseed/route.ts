...
// TEMPORARY. Re-embeds ALL tasks (picks up the 4 new commerce leaves) then runs
// Phase-1 self-match + dumps the commerce cluster. Remove after.

const CLUSTER = new Set([
  'payment-processing', 'process-refund', 'order-management',
  'product-catalog-management', 'ecommerce-management', 'manage-inventory',
...
export async function GET() {
  ...
  return NextResponse.json({ seeded, self_match_accuracy_pct: +(correct / tasks.length * 100).toFixed(1), failures, commerce_cluster: clusterRows })
}
