...
// TEMPORARY. Re-embeds ALL tasks (idempotent; picks up the 3 new browser tasks +
// the changed caption-images), then runs the Phase-1 self-match and dumps the
// browser cluster (each cluster task's example_query -> top-3 matched). Remove after.

const CLUSTER = new Set([
  'browser-automation', 'browser-form-automation', 'browser-screenshot',
  'browser-data-extraction', 'caption-images', 'e2e-testing', 'web-scraping',
])
...
export async function GET() {
  ...
  return NextResponse.json({
    seeded,
    self_match_accuracy_pct: +(correct / tasks.length * 100).toFixed(1),
    failures,
    browser_cluster: clusterRows,
  })
}
