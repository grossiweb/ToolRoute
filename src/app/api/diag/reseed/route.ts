...
// TEMPORARY. Re-embeds ALL tasks (picks up the 3 new devops/cloud leaves) then runs
// Phase-1 self-match + dumps the ops cluster. Remove after.

const CLUSTER = new Set([
  'cloud-deployment', 'infrastructure-as-code', 'dns-management',
  'infra-management', 'container-orchestration', 'error-monitoring', 'repo-management',
])
...
export async function GET() {
  ...
  return NextResponse.json({ seeded, self_match_accuracy_pct: +(correct / tasks.length * 100).toFixed(1), failures, ops_cluster: clusterRows })
}
