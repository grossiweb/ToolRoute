...
// TEMPORARY. Re-embeds ALL tasks (picks up the changed browser-screenshot example)
// then runs Phase-1 self-match + dumps the browser cluster. Remove after.
...
export async function GET() {
  ...
  return NextResponse.json({ seeded, self_match_accuracy_pct: +(correct / tasks.length * 100).toFixed(1), failures, browser_cluster: clusterRows })
}
