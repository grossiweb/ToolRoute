...
export async function POST(request: NextRequest) {
  ...
  const rl = rateLimit('skills-submit', rlKey, 10) // 10 server submissions/hour per IP
  if (!rl.allowed) {
    return NextResponse.json({ error: 'Rate limit exceeded. Max 10 server submissions per hour.' }, { status: 429 })
  }
