  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 24 }} className=" mb-8">
        {/* Stack name */}
        <div className="mb-4">
          <label className="text-xs font-semibold text-[var(--text-2)] uppercase tracking-wide mb-1 block">Stack Name</label>
          <input
            type="text"
        ...
        {/* Selected tools */}
        <label className="text-xs font-semibold text-[var(--text-2)] uppercase tracking-wide mb-2 block">
      ...
      {selected.length >= 1 && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 24 }} className=" mb-8 border-teal/20">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-lg">🚀</span>
                <h2 className="font-bold text-[var(--text)] text-lg">{stackName || 'My Stack'}</h2>
              </div>
              <p className="text-sm text-[var(--text-2)]">{selected.length} tool{selected.length !== 1 ? 's' : ''} &middot; Ready to deploy</p>
            </div>
            {avgScore != null && (
          ...
          <div className="space-y-2 mb-5">
            {selected.map(s => {
              ...
              const vs = normalize(sc.value_score ?? sc.overall_score)
              return (
                <div key={s.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-[var(--bg2)]">
                  <Link href={`/mcp-servers/${s.slug}`} className="text-sm font-medium text-[var(--text)] hover:text-brand transition-colors">
                    {s.canonical_name}
                  </Link>
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-[var(--text-3)]">Output {formatScore(normalize(sc.output_score) ?? 0)}</span>
                    <span className="text-[var(--text-3)]">Reliability {formatScore(normalize(sc.reliability_score) ?? 0)}</span>
                    <span className="text-[var(--text-3)]">Trust {formatScore(normalize(sc.trust_score) ?? 0)}</span>
          ...
          <div className="mb-3">
            <label className="text-xs font-semibold text-[var(--text-2)] uppercase tracking-wide mb-2 block">Deploy Config</label>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    configFormat === fmt
                      ...
                      : 'bg-[var(--bg3)] text-[var(--text-2)] hover:bg-[var(--bg3)]'
                  }`}
                >
            ...
            <div className="relative">
              ...
              <button
                onClick={copyConfig}
                className="absolute top-2 right-2 px-2.5 py-1 rounded-md bg-gray-700 text-[var(--text-3)] hover:bg-[var(--bg3)] text-xs font-medium transition-colors"
              >
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-[var(--border)]">
            ...
            <Link
              ...
              className="text-sm text-[var(--text-2)] hover:text-brand transition-colors ml-auto"
            >
              Compare scores →
            </Link>
          </div>
      ...
      {/* Popular Stacks */}
      <div className="mt-12">
        <h2 className="text-xl font-black text-[var(--text)] mb-2">Popular Stacks</h2>
        <p className="text-sm text-[var(--text-2)] mb-6">Pre-built tool combinations for common workflows. Click to customize and deploy.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {POPULAR_STACKS.map(stack => (
            <button
              ...
              style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 24 }} className=" group hover:border-brand/30 transition-all text-left"
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">🧩</span>
                <h3 className="font-bold text-[var(--text)] group-hover:text-brand transition-colors text-sm">
                  {stack.name}
                </h3>
              </div>
              <p className="text-xs text-[var(--text-2)] mb-3">{stack.description}</p>
              <div className="flex flex-wrap gap-1">
                {stack.tools.map(slug => (
                  <span key={slug} className="badge bg-[var(--bg3)] text-[var(--text-2)] text-[10px]">
      ...
      <div className="mt-8 rounded-xl border border-teal/20 bg-teal-light px-6 py-5 text-center">
        ...
        <code className="text-xs bg-[var(--bg)] px-3 py-1.5 rounded-lg border border-teal/20 text-[var(--text-2)] inline-block">
